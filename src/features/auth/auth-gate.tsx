...
export function AuthGate({ children }: PropsWithChildren) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setMessage(error ?? 'Check your inbox for a secure sign-in link.')
  }
  if (mode === 'loading')
    return (
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#0c0c0e] px-5 text-zinc-100">
      <form onSubmit={submit} className="w-full max-w-sm rounded-3xl bg-white/[0.04] p-8">
        ...
        <button className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-zinc-100 px-4 py-3 text-sm font-medium text-zinc-950">
